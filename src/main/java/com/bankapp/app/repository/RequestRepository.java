...
public interface RequestRepository extends CrudRepository<Requests, Integer> {
    ...
    @Query(value = "select * from requests where to_account = ?1 and status = false", nativeQuery = true)
    List<Requests> getAllPendingRequests(String accountNumber);

    @Query(value = "select * from requests where to_account = ?1 and status = true", nativeQuery = true)
